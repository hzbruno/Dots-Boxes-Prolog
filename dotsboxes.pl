...
haceFila(N, Fila) :-
    ...
    Fila =.. [fila|Celdas].                % construye fila(C1,…,CN), nos habilita a utilizar arg/3
...
jugada_humano(Tab, Turno, F, C, D, Tab2, Turno2, Celdas) :-
    functor(Tab, matriz, N1), N is N1,                      % obtengo N, tamaño matriz
    valid_move(Tab, F, C, D, N),                            % verifico si la jugada es válida
    marcoLinea(Tab, Turno, F, C, D, TabTemp, Celdas),        % maCo la linea
...
contar_puntos(Tab, P1, P2, C0) :-
    subtablero(Tab, Sub),
    functor(Sub, matriz, N1), N is N1,  % obtengo todas las celdas como términos c(H,V,J)
    findall(c(H,V,J),
            (
...
jugadas_posibles(Tab, Jugadas) :-
    functor(Tab, matriz, N1), N is N1,
    N2 is N-1,
    findall([F,C,h],
            ( between(1, N, F),
              between(1, N2, C),
              valid_move(Tab, F, C, h, N)
            ),
            Hs),
    findall([F,C,v],
            ( between(1, N2, F),
              between(1, N, C),
              valid_move(Tab, F, C, v, N)
            ),
            Vs),
    append(Hs, Vs, Jugadas).

% valid_move(+Tablero, +F, +C, +D, +N)
