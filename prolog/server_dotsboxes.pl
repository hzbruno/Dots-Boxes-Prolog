...
% Start this with:
%   swipl server_dotsboxes.pl --port 9999 --staticdir static

% TODO: if using daemon:
%         swipl server_dotsboxes.pl --port=.... --pidfile=/var/run/simple_server.pid
%       and kill $(cat /var/run/simple_server.pid)

% TODO: Support HTTPS: https://www.swi-prolog.org/pldoc/man?section=ssl-https-server

% See README.md for an overview of how the code works. dotsboxes

% Access this from a browser with
%   http://localhost:9999
%     which does a redirect to:
%   http://localhost:9999/static/dotsboxes.html
...
:- module(server_dotsboxes, [simple_server_main/0, simple_server_impl/1]).
...
% http_absolute_location(static('dotsboxes.js'), Path, []), Path='/static/dotsboxes.js'.
...
% http://localhost:9999/ ... redirects to /static/dotsboxes.html
